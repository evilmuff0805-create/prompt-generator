...
const SAFE_TOKEN_COUNT_KEY = /^(inputTokens|outputTokens|reasoningTokens|cachedInputTokens|totalTokens|promptTokens|completionTokens)$/;
const MAX_DEPTH = 6;
...
function sanitize(value, key = '', depth = 0, seen = new WeakSet()) {
  ...
  const safeTokenCount = SAFE_TOKEN_COUNT_KEY.test(keyName)
    && typeof value === 'number'
    && Number.isFinite(value);
  if (SENSITIVE_KEY.test(keyName) && !safeTokenCount) return REDACTED;
