...
function validateCreditPackConfiguration(env = process.env, reservedPriceIds = []) {
  ...
  if (
    enabled
    && String(env.PADDLE_SUBSCRIPTION_HISTORY_CONFIRMED || '')
      .trim()
      .toLowerCase() !== 'true'
  ) {
    const error = new Error(
      '[credit-packs] CREDIT_PACK_PURCHASES_ENABLED requires a Paddle API key with subscription_history.read'
    );
    error.code = 'CREDIT_PACK_SUBSCRIPTION_HISTORY_REQUIRED';
    throw error;
  }

  if (
    enabled
    && String(env.PADDLE_TRANSACTION_READ_CONFIRMED || '')
      .trim()
      .toLowerCase() !== 'true'
  ) {
    const error = new Error(
      '[credit-packs] CREDIT_PACK_PURCHASES_ENABLED requires a Paddle API key with transaction.read'
    );
    error.code = 'CREDIT_PACK_TRANSACTION_READ_REQUIRED';
    throw error;
  }
