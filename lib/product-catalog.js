...
function getPublicProductCatalog(env = process.env) {
  const storyboardCreditCost = getStoryboardCreditCost();
  const paddleEnvironment = validatePaddleEnvironmentCredentials(env);
  const configuredPaddleClientToken = typeof env.PADDLE_CLIENT_TOKEN === 'string'
  ...
  return {
    ...
    paddle: {
      clientToken: configuredPaddleClientToken || DEFAULT_PADDLE_CLIENT_TOKEN,
