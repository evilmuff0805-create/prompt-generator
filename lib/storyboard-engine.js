...
const { FACE_IDENTITY_CONSTRAINT } = require('./prompts/storyboard-constraints');
...
function validationErrorCode(error) {
  if (error.startsWith('Expected ')) return 'shot_count';
  if (error.startsWith('Camera angles not unique')) return 'camera_angle_unique';
  if (error.startsWith('Shot 5 must')) return 'shot_5_closeup';
  if (error.includes('missing "16:9"')) return 'aspect_ratio';
  if (error.includes('missing "cinematic 24fps"')) return 'frame_rate';
  // Keep the existing telemetry code stable even though Documentary now shares
  // the same realism contract.
  if (error.includes('missing "ultra-realistic"')) return 'cinematic_realism';
  if (error.includes('missing "preserve facial identity"')) return 'face_identity';
  if (error.includes('missing duration cue')) return 'duration_cue';
  if (error.includes('durationSeconds')) return 'duration_value';
  if (error.includes('exceeds Seedance')) return 'duration_total';
  return 'unknown';
}
...
function validateStoryboardData(data, cutCount, style) {
  ...
  shots.forEach((shot, i) => {
    ...
    if (STYLE_TEMPLATES[style]?.realismConstraint && !prompt.toLowerCase().includes('ultra-realistic')) {
      errors.push(`Shot ${i + 1}: missing "ultra-realistic" for ${style} style`);
    }
    if (!prompt.toLowerCase().includes('preserve facial identity')) {
      errors.push(`Shot ${i + 1}: missing "preserve facial identity"`);
    }
...
async function generateScenarioAndPrompts({ scenario, genres, style, cutCount }) {
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    ...
    // Do not log generated text. Stable codes are sufficient for regression telemetry.
    const constraintCodes = [...new Set(errors.map(validationErrorCode))];
    logger.warn('storyboard.text.constraint_failed', {
      provider: 'openai',
      attempt,
      maxAttempts: 2,
  ...
  const finalConstraintCodes = [...new Set(lastValidationErrors.map(validationErrorCode))];
  const err = new Error(`Storyboard validation failed after 2 attempts: ${finalConstraintCodes.join(', ')}`);
...
function buildGridPrompt(data, style, cutCount) {
  ...
  return `Create a ${cutCount === 9 ? '3×3' : '2×2'} storyboard grid image. ${cutCount} panels arranged left-to-right, top-to-bottom. Each panel shows a distinct cinematic shot. ${styleDirective}${continuityDirective}${frameSafetyDirective} Mandatory recurring-face consistency: ${FACE_IDENTITY_CONSTRAINT}. Apply this only when a face is visible. 16:9 aspect ratio per panel. No text, labels, or numbers in the image. High quality.
