...
const { recordServerEvent } = require('./product-analytics');
...
function durationBucket(durationMs) {
  if (durationMs < 30000) return '<30s';
  if (durationMs < 60000) return '30-60s';
  if (durationMs < 120000) return '60-120s';
  if (durationMs < 300000) return '120-300s';
  return '300s+';
}
