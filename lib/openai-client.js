...
const STORYBOARD_PROMPT_VERSION = process.env.STORYBOARD_PROMPT_VERSION || 'storyboard-v3';
const STORYBOARD_GRID_PROMPT_VERSION = process.env.STORYBOARD_GRID_PROMPT_VERSION || 'storyboard-grid-v2';
...
async function generateStoryboardData({ scenario, genres, style, cutCount, systemPrompt }) {
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    ...
    let schemaResult = 'not_evaluated';
    try {
      ...
      let data;
      try {
        data = normalizeStoryboardData(parsed);
        schemaResult = 'passed';
      } catch (schemaError) {
        schemaResult = 'failed';
        throw schemaError;
      ...
      lastError = err;
      const retryScheduled = attempt === 1 && err instanceof SyntaxError;
      recordAiCall({
        outcome: 'failed',
        provider: 'openai',
        operation: 'storyboard.text',
...
async function generateStoryboardGrid({ prompt, refImageBuffers, cutCount }) {
  ...
  const startedAt = Date.now();
  const operation = hasRefs ? 'storyboard.grid.edit' : 'storyboard.grid.generate';
  ...
  try {
    ...
    recordAiCall({
      ...
      operation,
      model: IMAGE_MODEL,
      promptVersion: STORYBOARD_GRID_PROMPT_VERSION,
      startedAt,
      usage: extractOpenAIUsage(response.usage),
      responseId: response.id
    });
