    ...
    ko: {
      terms: {
        ...
        html: documentHtml(
          ...
          <p>검색 비노출 스토리보드 공유 링크를 사용자가 직접 만들면, 해당 링크를 비공개 처리하거나 원본 스토리보드가 만료될 때까지 선택한 생성 그리드와 기본 표시 정보만 정확한 링크를 가진 사람에게 제공하도록 당사에 지시하는 것입니다. 사용자는 그리드를 공유할 권리가 있음을 확인합니다. 이 기능은 PromptGen에 홍보 또는 갤러리 사용 권한을 부여하지 않습니다.</p>
          <p>PromptGen의 명칭, 로고, 웹사이트 디자인 및 기반 기술은 당사의 지식재산이며 서면 허가 없이 복사하거나 재사용할 수 없습니다.</p>
          <h2>6. 결제 및 크레딧</h2>
          <p>Pro와 Enterprise 반복 월간 구독 및 표시된 일회성 PromptGen 사용량 추가 구매는 당사의 판매 책임자(Merchant of Record)이자 공인 리셀러인 <strong>Paddle</strong>이 판매·처리합니다. 구독은 취소할 때까지 자동으로 갱신됩니다. 사용량 추가 구매는 활성 유료 구독자에게만 제공되고 구독이 아니며, 월간 플랜 크레딧과 별도로 관리되고 구매 후 365일에 만료됩니다. 가격은 관련 세금 적용 전 USD로 표시되며, Paddle Checkout은 구매 전에 최종 현지화 금액과 세금 처리를 보여 줍니다.</p>
          <p>Paddle 고객 포털에서 구독을 관리하거나 취소할 수 있습니다. 취소는 현재 결제 기간 종료 시 효력이 발생합니다. 모든 크레딧은 PromptGen 내부 사용량이며 양도, 재판매, 현금 교환 또는 인출할 수 없습니다. 구독 취소 후에도 만료되지 않은 추가 구매 크레딧은 잔액으로 남을 수 있지만, 다시 활성 유료 구독 상태가 될 때까지 사용할 수 없습니다. 환불 및 법정 철회권에는 당사의 <a href="/refund.html">환불 정책</a>, Paddle의 구매자 약관과 환불 정책, 관련 법률이 적용됩니다.</p>
          <h2>7. 보증의 부인</h2>
      ...
      privacy: {
        ...
        html: documentHtml(
          ...
          <ul>
            ...
            <li><strong>검색 비노출 스토리보드 링크:</strong> 사용자가 직접 만들면 생성 그리드와 시각 스타일, 컷 수, 생성일, 만료일만 정확한 링크를 가진 사람에게 보입니다. 링크를 비공개 처리하거나 원본 스토리보드가 만료되면 접근이 종료됩니다. 시나리오, 컷 프롬프트, 레퍼런스 이미지와 계정 신원은 포함되지 않습니다. 당사는 링크 토큰의 단방향 해시, 동의 버전과 시각, 만료 및 철회 상태를 저장하며 원본 토큰은 저장하지 않습니다.</li>
      ...
      refund: {
        ...
        html: documentHtml(
          ...
          koNotice,
          `
          <div class="highlight-box"><p>PromptGen 구독과 일회성 사용량 추가 구매는 당사의 판매 책임자(Merchant of Record)인 <strong>Paddle</strong>이 판매합니다. 환불 가능 여부는 <a href="https://www.paddle.com/legal/refund-policy" rel="noopener noreferrer">Paddle 환불 정책</a>과 관련 소비자법에 따라 결정됩니다. 본 정책은 사용자 국가에서 보장되는 강행적 권리를 제한하지 않습니다.</p></div>
          <hr class="divider" />
          <h2>1. 구독 및 사용량 추가 구매</h2>
          <p>Pro와 Enterprise는 월간 구독입니다. Pro는 결제 주기마다 600크레딧, Enterprise는 1,500크레딧을 제공합니다. 갱신 시 월간 플랜 크레딧은 플랜 할당량으로 초기화되며 미사용분은 이월되지 않습니다. 활성 유료 구독자가 구매한 일회성 사용량 추가 구매는 월간 크레딧과 별도로 관리되고 구매 후 365일에 만료되며, 양도하거나 현금으로 교환할 수 없습니다.</p>
          <h2>2. 환불 가능 조건</h2>
          ...
          <h2>5. 결제 처리업체</h2>
          <p>Paddle은 PromptGen 구독과 일회성 사용량 추가 구매의 판매자이자 판매 책임자입니다. Paddle이 결제 지원, 구독 취소 및 환불을 처리합니다. PromptGen은 사용자의 전체 결제 카드 정보를 저장하지 않습니다.</p>
          <h2>6. 정책 변경</h2>
    ...
    ja: {
      terms: {
        ...
        html: documentHtml('利用規約', '施行日：2026年5月1日 · 最終更新：2026年7月24日',
          ...
          <h2>6. 支払いとクレジット</h2><p>ProとEnterpriseの月額自動更新サブスクリプション、および表示された1回限りのPromptGen追加利用枠は、当社のMerchant of Record（販売責任者）兼認定再販業者である<strong>Paddle</strong>が販売・処理します。サブスクリプションは解約するまで自動更新されます。追加利用枠は有効な有料契約者にのみ提供され、サブスクリプションではなく、月額クレジットとは別に管理され、購入から365日後に失効します。価格は適用税前のUSDで表示され、Paddle Checkoutが購入前に最終的な現地通貨額と税処理を表示します。</p><p>Paddleのカスタマーポータルでサブスクリプションを管理・解約できます。解約は現在の請求期間終了時に有効になります。すべてのクレジットはPromptGen内の利用枠で、譲渡、再販売、現金への交換、出金はできません。解約後も未失効の追加購入分が残る場合がありますが、有効な有料契約を再開するまで使用できません。返金と法定撤回権には、当社の<a href="/refund.html">返金ポリシー</a>、Paddleの購入者条件および返金ポリシー、適用法が適用されます。</p>
      ...
      privacy: {
        ...
        html: documentHtml('プライバシーポリシー', '施行日：2026年5月1日 · 最終更新：2026年7月21日',
          'この翻訳は便宜のために提供されています。翻訳文と英語原文が矛盾する場合、適用法で許される範囲において英語原文が優先されます。', `
          ...
          <h2>4. データ保持</h2><ul><li><strong>アカウントデータ</strong>はアカウントが有効な間保持されます。</li><li><strong>画像→プロンプトのアップロード</strong>は分析後に削除されます。</li><li><strong>ストーリーボードの参考画像</strong>は処理のため非公開保存され、24時間後に期限切れとなります。</li><li><strong>生成されたストーリーボードグリッド</strong>は作成日から90日間履歴で利用でき、その後定期削除されます。soft-deleteされたデータベース記録は完全削除まで最大30日追加で残る場合があります。</li><li><strong>限定公開ストーリーボードリンク：</strong>自ら作成した場合、生成グリッド、ビジュアルスタイル、ショット数、作成日、期限だけが正確なリンクの所持者に表示されます。非公開化または元のストーリーボードの期限でアクセスは終了します。シナリオ、ショットプロンプト、参考画像、アカウント情報は含まれません。当社はリンクトークンの一方向ハッシュ、同意の版と時刻、期限、失効状態を保存し、生のトークンは保存しません。</li><li><strong>プロンプト履歴</strong>は削除または退会まで保存されます。</li><li><strong>支払い記録</strong>は法令上必要な期間保持されます。</li><li><strong>自社製品分析イベント</strong>は180日後に自動削除されます。</li></ul>
      ...
      refund: {
        ...
        html: documentHtml('返金ポリシー', '施行日：2026年5月1日 · 最終更新：2026年7月24日',
          'この翻訳は便宜のために提供されています。翻訳文と英語原文が矛盾する場合、適用法で許される範囲において英語原文が優先されます。', `
          <div class="highlight-box"><p>PromptGenのサブスクリプションと1回限りの追加利用枠はMerchant of Recordである<strong>Paddle</strong>が販売します。返金可否は<a href="https://www.paddle.com/legal/refund-policy" rel="noopener noreferrer">Paddleの返金ポリシー</a>と適用される消費者法に基づきます。本ポリシーは、お住まいの国で認められる強行的権利を制限しません。</p></div><hr class="divider" />
          <h2>1. サブスクリプションと追加利用枠</h2><p>ProとEnterpriseは月額サブスクリプションです。請求サイクルごとにProは600クレジット、Enterpriseは1,500クレジットを付与し、更新時に月額分はプラン割当数へリセットされ、未使用分は繰り越されません。有効な有料契約者が購入する1回限りの追加利用枠は月額分とは別に管理され、購入から365日後に失効し、譲渡や換金はできません。</p>
          ...
          <h2>5. 決済処理業者</h2><p>PaddleはPromptGenのサブスクリプションと1回限りの追加利用枠の販売者兼Merchant of Recordで、決済サポート、サブスクリプションの解約、返金を扱います。PromptGenはカード番号全体を保存しません。</p>
    ...
    'zh-CN': {
      terms: {
        ...
        html: documentHtml('服务条款', '生效日期：2026年5月1日 · 最后更新：2026年7月24日',
          ...
          <h2>6. 付款与积分</h2><p>Pro 和 Enterprise 循环月度订阅以及页面所示的一次性 PromptGen 用量加购，均由我们的记录商户（Merchant of Record）及授权经销商 <strong>Paddle</strong> 销售和处理。订阅会自动续费，直至取消。用量加购仅向有效付费订阅用户提供，不属于订阅，与月度积分分开管理，并在购买后365天到期。价格在适用税费前以美元（USD）显示，Paddle Checkout 会在购买前显示最终本地化金额和税务处理。</p><p>您可通过 Paddle 客户门户管理或取消订阅。取消将在当前计费周期结束时生效。所有积分仅代表 PromptGen 内部使用量，不可转让、转售、兑换现金或提现。取消订阅后，未到期的加购积分可能仍会保留，但在重新拥有有效付费订阅前无法使用。退款和法定撤回权受我们的<a href="/refund.html">退款政策</a>、Paddle 买方条款与退款政策以及适用法律管辖。</p>
      ...
      privacy: {
        ...
        html: documentHtml('隐私政策', '生效日期：2026年5月1日 · 最后更新：2026年7月21日',
          '本译文仅为方便阅读而提供。如译文与英文原文存在冲突，在适用法律允许的范围内，以英文原文为准。', `
          ...
          <h2>4. 数据保留</h2><ul><li><strong>账号数据</strong>在账号有效期间保留。</li><li><strong>图片转提示词上传</strong>在分析后删除。</li><li><strong>故事板参考图</strong>为处理需要存于私有存储，并在24小时后过期。</li><li><strong>生成的故事板网格</strong>自创建日起在历史记录中保留90天，之后由定期任务删除。soft-delete 的数据库记录在永久删除前可能额外保留最多30天。</li><li><strong>不公开列出的故事板链接：</strong>主动创建后，仅生成网格、视觉风格、镜头数、创建日期和到期日期可由拥有准确链接的人查看；取消共享或源故事板过期后访问即终止。剧本、镜头提示词、参考图片和账户身份不会包含在内。我们保存链接令牌的单向哈希、同意版本与时间、到期和撤销状态，不保存原始令牌。</li><li><strong>提示词历史</strong>保存至您删除记录或关闭账号。</li><li><strong>付款记录</strong>按适用法律要求保留。</li><li><strong>自有产品分析事件</strong>保留180天后自动删除。</li></ul>
      ...
      refund: {
        ...
        html: documentHtml('退款政策', '生效日期：2026年5月1日 · 最后更新：2026年7月24日',
          '本译文仅为方便阅读而提供。如译文与英文原文存在冲突，在适用法律允许的范围内，以英文原文为准。', `
          <div class="highlight-box"><p>PromptGen 订阅和一次性用量加购均由我们的记录商户（Merchant of Record）<strong>Paddle</strong> 销售。退款资格根据 <a href="https://www.paddle.com/legal/refund-policy" rel="noopener noreferrer">Paddle 退款政策</a>和适用消费者法律确定。本政策不限制您所在国家提供的强制性权利。</p></div><hr class="divider" />
          <h2>1. 订阅与用量加购</h2><p>Pro 和 Enterprise 是月度订阅。Pro 每个计费周期提供600积分，Enterprise 提供1,500积分；续订时月度积分重置为方案额度，未使用部分不结转。有效付费订阅用户购买的一次性用量加购与月度积分分开管理，在购买后365天到期，且不可转让或兑换现金。</p>
          ...
          <h2>5. 付款处理方</h2><p>Paddle 是 PromptGen 订阅和一次性用量加购的卖方及记录商户，负责付款支持、订阅取消和退款。PromptGen 不存储完整的银行卡资料。</p>
    ...
    fr: {
      terms: {
        ...
        html: documentHtml('Conditions d’utilisation', 'Date d’entrée en vigueur : 1 mai 2026 · Dernière mise à jour : 24 juillet 2026',
          ...
          <h2>6. Paiements et crédits</h2><p>Les abonnements mensuels récurrents Pro et Enterprise, ainsi que les achats uniques d’usage PromptGen affichés, sont vendus et traités par <strong>Paddle</strong>, notre Merchant of Record et revendeur agréé. Les abonnements se renouvellent automatiquement jusqu’à résiliation. Les achats d’usage sont réservés aux abonnés payants actifs, ne constituent pas un abonnement, sont gérés séparément des crédits mensuels et expirent 365 jours après l’achat. Les prix sont affichés en USD avant taxes ; Paddle Checkout présente avant l’achat le montant final localisé et le traitement fiscal.</p><p>Vous pouvez gérer ou résilier l’abonnement depuis le portail client Paddle. La résiliation prend effet à la fin de la période en cours. Tous les crédits représentent un usage interne de PromptGen et ne peuvent être transférés, revendus, convertis en espèces ou retirés. Des crédits supplémentaires non expirés peuvent rester après résiliation, mais ne sont utilisables qu’après réactivation d’un abonnement payant. Les remboursements et droits de rétractation obligatoires sont régis par notre <a href="/refund.html">Politique de remboursement</a>, les conditions acheteur et la politique de Paddle, ainsi que la loi applicable.</p>
      ...
      privacy: {
        ...
        html: documentHtml('Politique de confidentialité', 'Date d’entrée en vigueur : 1 mai 2026 · Dernière mise à jour : 21 juillet 2026',
          'Cette traduction est fournie à titre pratique. En cas de divergence avec la version anglaise, celle-ci prévaut dans la mesure autorisée par la loi applicable.', `
          ...
          <h2>4. Conservation</h2><ul><li>Les <strong>données de compte</strong> restent conservées tant que le compte est actif.</li><li>Les <strong>images Image vers prompt</strong> sont supprimées après analyse.</li><li>Les <strong>références de storyboard</strong> sont conservées en stockage privé pour le traitement et expirent après 24 heures.</li><li>Les <strong>grilles de storyboard générées</strong> restent disponibles dans l’historique pendant 90 jours après leur création, puis sont supprimées par le nettoyage planifié. L’enregistrement de base de données soft-delete peut subsister jusqu’à 30 jours supplémentaires avant sa suppression définitive.</li><li><strong>Liens de storyboard non répertoriés :</strong> si vous en créez un, seuls la grille, le style visuel, le nombre de plans et les dates de création et d’expiration sont visibles avec le lien exact. L’accès cesse lors de la désactivation ou de l’expiration du storyboard source. Le scénario, les prompts, les références et l’identité du compte sont exclus. Nous conservons un hachage à sens unique du jeton, la version et l’heure du consentement, l’expiration et l’état de révocation, mais pas le jeton brut.</li><li>L’<strong>historique des prompts</strong> reste jusqu’à sa suppression ou la fermeture du compte.</li><li>Les <strong>paiements</strong> sont conservés selon la loi.</li><li>Les <strong>événements d’analyse internes</strong> sont supprimés automatiquement après 180 jours.</li></ul>
      ...
      refund: {
        ...
        html: documentHtml('Politique de remboursement', 'Date d’entrée en vigueur : 1 mai 2026 · Dernière mise à jour : 24 juillet 2026',
          'Cette traduction est fournie à titre pratique. En cas de divergence avec la version anglaise, celle-ci prévaut dans la mesure autorisée par la loi applicable.', `
          <div class="highlight-box"><p>Les abonnements PromptGen et les achats uniques d’usage sont vendus par <strong>Paddle</strong>, notre Merchant of Record. L’éligibilité dépend de la <a href="https://www.paddle.com/legal/refund-policy" rel="noopener noreferrer">politique de remboursement de Paddle</a> et du droit de la consommation applicable. Rien ici ne limite les droits obligatoires de votre pays.</p></div><hr class="divider" />
          <h2>1. Abonnements et achats d’usage</h2><p>Pro et Enterprise sont des abonnements mensuels. Pro fournit 600 crédits et Enterprise 1 500 par cycle ; au renouvellement, les crédits mensuels reviennent au quota et le solde inutilisé n’est pas reporté. Les achats uniques d’usage réservés aux abonnés payants actifs sont gérés séparément, expirent 365 jours après l’achat et ne sont ni transférables ni convertibles en espèces.</p>
          ...
          <h2>5. Prestataire de paiement</h2><p>Paddle est le vendeur et Merchant of Record des abonnements PromptGen et des achats uniques d’usage. Il gère l’assistance paiement, la résiliation des abonnements et les remboursements. PromptGen ne stocke pas les données complètes de votre carte.</p>
    ...
    ru: {
      terms: {
        ...
        html: documentHtml('Условия использования', 'Дата вступления в силу: 1 мая 2026 г. · Последнее обновление: 24 июля 2026 г.',
          ...
          <h2>6. Платежи и кредиты</h2><p>Повторяющиеся ежемесячные подписки Pro и Enterprise, а также указанные разовые покупки объёма использования PromptGen продаёт и обрабатывает <strong>Paddle</strong>, наш Merchant of Record и уполномоченный реселлер. Подписки автоматически продлеваются до отмены. Разовый объём предлагается только активным платным подписчикам, не является подпиской, учитывается отдельно от ежемесячных кредитов и истекает через 365 дней после покупки. Цены показаны в USD до налогов; Paddle Checkout до покупки показывает итоговую локализованную сумму и налоги.</p><p>Управлять подпиской и отменять её можно в клиентском портале Paddle. Отмена вступает в силу в конце текущего периода. Все кредиты представляют внутренний объём использования PromptGen и не могут передаваться, перепродаваться, обмениваться на деньги или выводиться. Неистёкшие дополнительные кредиты могут сохраниться после отмены, но использовать их можно только после возобновления платной подписки. Возвраты и обязательные права отказа регулируются нашей <a href="/refund.html">Политикой возврата</a>, условиями покупателя и политикой Paddle, а также законом.</p>
      ...
      privacy: {
        ...
        html: documentHtml('Политика конфиденциальности', 'Дата вступления в силу: 1 мая 2026 г. · Последнее обновление: 21 июля 2026 г.',
          'Перевод предоставлен для удобства. При расхождениях с английской версией она имеет преимущественную силу в пределах, допускаемых применимым законодательством.', `
          ...
          <h2>4. Сроки хранения</h2><ul><li><strong>Аккаунт</strong> — пока он активен.</li><li><strong>Изображения для анализа</strong> — удаляются после анализа.</li><li><strong>Референсы раскадровки</strong> хранятся конфиденциально для обработки и истекают через 24 часа.</li><li><strong>Созданные сетки раскадровки</strong> доступны в истории 90 дней с даты создания, после чего удаляются плановой очисткой. Запись базы данных в состоянии soft-delete может сохраняться ещё до 30 дней до окончательного удаления.</li><li><strong>Непубличные ссылки на раскадровку:</strong> после намеренного создания по точной ссылке видны только сетка, визуальный стиль, число кадров и даты создания и истечения. Доступ прекращается при отключении или истечении исходной раскадровки. Сценарий, промпты, референсы и личность владельца не включаются. Мы храним односторонний хэш токена, версию и время согласия, срок действия и статус отзыва, но не исходный токен.</li><li><strong>История промптов</strong> — до удаления или закрытия аккаунта.</li><li><strong>Платежи</strong> — в сроки, требуемые законом.</li><li><strong>События собственной аналитики</strong> — 180 дней, затем автоматически удаляются.</li></ul>
      ...
      refund: {
        ...
        html: documentHtml('Политика возврата', 'Дата вступления в силу: 1 мая 2026 г. · Последнее обновление: 24 июля 2026 г.',
          'Перевод предоставлен для удобства. При расхождениях с английской версией она имеет преимущественную силу в пределах, допускаемых применимым законодательством.', `
          <div class="highlight-box"><p>Подписки PromptGen и разовые покупки объёма использования продаёт <strong>Paddle</strong>, наш Merchant of Record. Право на возврат определяется <a href="https://www.paddle.com/legal/refund-policy" rel="noopener noreferrer">Политикой возврата Paddle</a> и применимым законодательством о защите потребителей. Ничто здесь не ограничивает обязательные права в вашей стране.</p></div><hr class="divider" />
          <h2>1. Подписки и дополнительный объём</h2><p>Pro и Enterprise — ежемесячные подписки. Pro даёт 600 кредитов, Enterprise — 1 500 за цикл; при продлении ежемесячный баланс сбрасывается до квоты, остаток не переносится. Разовый дополнительный объём для активных платных подписчиков учитывается отдельно, истекает через 365 дней после покупки и не может передаваться или обмениваться на деньги.</p>
          ...
          <h2>5. Платёжный оператор</h2><p>Paddle — продавец и Merchant of Record подписок PromptGen и разовых покупок объёма использования, отвечающий за поддержку платежей, отмену подписок и возвраты. PromptGen не хранит полные данные карты.</p>
