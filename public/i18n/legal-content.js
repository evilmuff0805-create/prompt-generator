      ...
      privacy: {
        ...
        html: documentHtml(
          ...
          <ul>
            ...
            <li><strong>스토리보드 레퍼런스 및 생성 그리드</strong>는 결과 보존 기간 동안 비공개 저장소에 유지되며 완료·실패 상태의 스토리보드가 만료된 뒤 예약 정리로 삭제됩니다(기본 30일).</li>
    ...
    ja: {
      ...
      privacy: {
        ...
        html: documentHtml('プライバシーポリシー', '施行日：2026年5月1日 · 最終更新：2026年7月18日',
          ...
          <h2>4. データ保持</h2><ul><li><strong>アカウントデータ</strong>はアカウントが有効な間保持されます。</li><li><strong>画像→プロンプトのアップロード</strong>は分析後に削除されます。</li><li><strong>参考画像と生成グリッド</strong>は結果保持期間中に非公開保存され、完了・失敗したストーリーボードの期限後、定期削除されます（既定30日）。</li><li><strong>プロンプト履歴</strong>は削除または退会まで保存されます。</li><li><strong>支払い記録</strong>は法令上必要な期間保持されます。</li><li><strong>自社製品分析イベント</strong>は180日後に自動削除されます。</li></ul>
    ...
    'zh-CN': {
      ...
      privacy: {
        ...
        html: documentHtml('隐私政策', '生效日期：2026年5月1日 · 最后更新：2026年7月18日',
          ...
          <h2>4. 数据保留</h2><ul><li><strong>账号数据</strong>在账号有效期间保留。</li><li><strong>图片转提示词上传</strong>在分析后删除。</li><li><strong>故事板参考图和生成网格</strong>在结果保留期内存于私有存储，处于完成或失败状态的故事板到期后由定期任务删除（默认30天）。</li><li><strong>提示词历史</strong>保存至您删除记录或关闭账号。</li><li><strong>付款记录</strong>按适用法律要求保留。</li><li><strong>自有产品分析事件</strong>保留180天后自动删除。</li></ul>
    ...
    ru: {
      ...
      privacy: {
        ...
        html: documentHtml('Политика конфиденциальности', 'Дата вступления в силу: 1 мая 2026 г. · Последнее обновление: 18 июля 2026 г.',
          ...
          <h2>4. Сроки хранения</h2><ul><li><strong>Аккаунт</strong> — пока он активен.</li><li><strong>Изображения для анализа</strong> — удаляются после анализа.</li><li><strong>Референсы и сетки</strong> — в закрытом хранилище на период доступности результата и удаляются после истечения завершённых/неудачных раскадровок (по умолчанию 30 дней).</li><li><strong>История промптов</strong> — до удаления или закрытия аккаунта.</li><li><strong>Платежи</strong> — в сроки, требуемые законом.</li><li><strong>События собственной аналитики</strong> — 180 дней, затем автоматически удаляются.</li></ul>
